...
const FilterSidebar: React.FC<FilterSidebarProps> = ({ 
  ...
  return (
    <div className={`${isMobile ? 'w-full' : 'lg:w-64'} flex-shrink-0 mb-4 lg:mb-0 lg:mr-6`}>
      {/* Filter Toggle Button */}
      <button 
        id="filters-toggle"
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full bg-white rounded-lg shadow p-3 sm:p-4 flex justify-between items-center mb-2"
      >
        <div className="flex items-center">
          <Filter className={`${isMobile ? 'h-4 w-4' : 'h-5 w-5'} mr-2 text-secondary`} />
          <span className={`font-heading font-semibold ${isMobile ? 'text-base' : 'text-lg'} text-secondary`}>
            Filters
          </span>
          {activeFilterCount > 0 && (
        ...
        {isExpanded ? (
          <ChevronUp className={`${isMobile ? 'h-4 w-4' : 'h-5 w-5'} text-secondary`} />
        ) : (
          <ChevronDown className={`${isMobile ? 'h-4 w-4' : 'h-5 w-5'} text-secondary`} />
        )}
      </button>
      ...
      {isExpanded && (
        <div className="bg-white rounded-lg shadow p-3 sm:p-4">
          {/* Mobile Layout - Two-column grid for filters */}
          {isMobile && (
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <div className="mb-4">
                  <h4 className="font-medium text-secondary text-sm mb-2">Pace Category</h4>
                  <div className="space-y-1">
                    <label className="flex items-center text-sm">
                      <input 
                        type="checkbox" 
                        className="rounded h-3 w-3 text-beginner focus:ring-beginner" 
                      ...
                      <span className="ml-1 inline-flex items-center">
                        <span className="inline-block w-2 h-2 rounded-full bg-beginner mr-1"></span>
                    ...
                    <label className="flex items-center text-sm">
                      <input 
                        type="checkbox" 
                        className="rounded h-3 w-3 text-intermediate focus:ring-intermediate" 
                      ...
                      />
                      <span className="ml-1 inline-flex items-center">
                        <span className="inline-block w-2 h-2 rounded-full bg-intermediate mr-1"></span>
                    ...
                    <label className="flex items-center text-sm">
                      <input 
                        type="checkbox" 
                        className="rounded h-3 w-3 text-advanced focus:ring-advanced" 
                        checked={localFilters.paceCategories?.includes('advanced')}
                        onChange={(e) => handlePaceCategoryChange('advanced', e.target.checked)}
                      />
                      <span className="ml-1 inline-flex items-center">
                        <span className="inline-block w-2 h-2 rounded-full bg-advanced mr-1"></span>
                ...
                <div className="mb-4">
                  <h4 className="font-medium text-secondary text-sm mb-2">Distance</h4>
                  <div className="space-y-1">
                    <label className="flex items-center text-sm">
                      <input 
                        type="checkbox" 
                        className="rounded h-3 w-3 text-primary focus:ring-primary" 
                        checked={localFilters.distanceRanges?.includes('short')}
                        onChange={(e) => handleDistanceRangeChange('short', e.target.checked)}
                      />
                      <span className="ml-1">{getDistanceRangeLabel('short')}</span>
                    </label>
                    <label className="flex items-center text-sm">
                      <input 
                        type="checkbox" 
                      ...
                      />
                      <span className="ml-1">{getDistanceRangeLabel('medium')}</span>
                    </label>
                    <label className="flex items-center text-sm">
                      <input 
                        type="checkbox" 
                      ...
                      />
                      <span className="ml-1">{getDistanceRangeLabel('long')}</span>
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div>
                {/* Other Filters */}
                <div className="mb-4">
                  <h4 className="font-medium text-secondary text-sm mb-2">Special Filters</h4>
                  <div className="space-y-1">
                    <label className="flex items-center text-sm">
                      <input 
                        type="checkbox" 
                      ...
                      />
                      <span className="ml-1">Beginner friendly</span>
                    </label>
                    ...
                    <label className="flex items-center text-sm">
                      ...
                      />
                      <span className="ml-1">Interval training</span>
                    </label>
                  </div>
                </div>
                ...
                {/* Clubs Filter - Mobile */}
                <div className="mb-4">
                  <h4 className="font-medium text-secondary text-sm mb-2">Running Clubs</h4>
                  <div className="space-y-1">
                    {displayedClubs.slice(0, 3).map(club => (
                      <label key={club.id} className="flex items-center text-sm">
                        <input 
                          type="checkbox" 
                          className="rounded h-3 w-3 text-primary focus:ring-primary" 
                        ...
                        />
                        <span className="ml-1 truncate" style={{ maxWidth: '120px' }}>{club.name}</span>
                      </label>
                    ))}
                  </div>
                  {clubs.length > 3 && (
          ...
          {/* Desktop Layout - Standard column layout */}
          {!isMobile && (
            <>
              {/* Pace Category Filter */}
              <div className="mb-4">
          ...
          {/* Filter buttons - Common for mobile and desktop */}
          <div className="flex flex-col sm:flex-row sm:space-x-2 space-y-2 sm:space-y-0">
            <button 
              className="w-full bg-primary hover:bg-opacity-90 text-white font-medium py-2 px-4 rounded text-sm"
            ...
            {activeFilterCount > 0 && (
              <button 
                className="w-full border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium py-2 px-4 rounded text-sm"
